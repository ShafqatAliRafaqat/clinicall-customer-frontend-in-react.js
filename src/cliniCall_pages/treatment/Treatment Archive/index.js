...
class TreatmentList extends Component {
  ...
  getArchiveTreatment = async (page) => {
    try {
      let url = "admin/deleted_treatment?page=1";
      if (page) url = `admin/deleted_treatment?page=${page}`;
      this.setState({
        isLoading: true,
      });
  ...
  handleRestore = async () => {
    try {
      this.confirmRestoreModalToggle();
      let url = "admin/restore_treatment";
      this.setState({
        isLoading: true,
      });
  ...
  handlePagination = (page) => {
    ...
    this.getArchiveTreatment(page.selected + 1);
  };
    ...
    return (
      <React.Fragment>
        ...
        >
          ...
          <ModalBody>Do you really want to restore Treatment(s)?</ModalBody>
          <ModalFooter>
