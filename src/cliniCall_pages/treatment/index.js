...
import Sidebar from "./TreatmentForm";
import SidebarView from "./TreatmentView";
...
const CustomHeader = (props) => {
  return (
    <div>
      <Breadcrumbs breadCrumbTitle="Treatments" breadCrumbActive="Treatments" />
...
class TreatmentList extends Component {
  ...
  getParentTreatments = async () => {
    try {
      let url = "admin/parent_treatments";
      this.setState({
        isLoading: true,
      });
      const res = await apiCall("get", url, undefined, this.state.token);
      this.setState({ parent_treatments: res.data.data, isLoading: false });
    } catch (err) {
      if (err.response)
  ...
  getTreatments = async (page) => {
    try {
      let url = "admin/treatment?page=1";
      if (page && this.props.userData.actor !== "DOCTOR")
        url = `admin/treatment?page=${page}`;
      else if (this.props.userData.actor === "DOCTOR")
        url = `auth/doctor_treatments/${this.props.userData.profile.id}?page=1`;
      else if (page && this.props.userData.actor === "DOCTOR")
        url = `auth/doctor_treatments/${this.props.userData.profile.id}?page=${page}`;
      this.setState({
        isLoading: true,
      });
      const res = await apiCall("get", url, undefined, this.state.token);
  ...
  handleDelete = async () => {
    try {
      this.confirmDeleteModalToggle();
      let url = "admin/delete_treatment";
      this.setState({
        isLoading: true,
      });
  ...
  handlePagination = (page) => {
    let urlPrefix = "/treatments";
    history.push(`${urlPrefix}?page=${page.selected + 1}`);
    this.setState({ currentPage: page.selected });
    this.getTreatments(page.selected + 1);
  };
    ...
    return (
      <React.Fragment>
          ...
          >
            ...
            {parent_treatments.length && (
              <Sidebar
                ...
                handleSidebar={this.handleSidebar}
                getData={this.getTreatments}
                addNew={this.state.addNew}
                parent_treatments={parent_treatments}
              />
            )}
            <SidebarView
              show={sidebarDetails}
        ...
        >
          ...
          <ModalBody>Do you really want to delete Treatment(s)?</ModalBody>
          <ModalFooter>
