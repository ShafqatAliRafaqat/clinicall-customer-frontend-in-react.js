...
class PatientArchiveList extends Component {
  state = {
    data: [],
    totalPages: 0,
    currentPage: 0,
    ...
    columns: [
      ...
      },
      {
        ...
        selector: "location",
        sortable: true,
        cell: (row) => `${row.city_id.name},${row.country_code.name}`,
      },
      {
        name: "Actions",
        sortable: false,
  ...
  getOrganizations = async (page) => {
    try {
      let url = "admin/deleted_organization?page=1";
      if (page) url = `admin/deleted_organization?page=${page}`;
      this.setState({
        isLoading: true,
      });
  ...
  handleRestore = async () => {
    try {
      this.confirmRestoreModalToggle();
      let url = "admin/restore_organization";
      this.setState({
        isLoading: true,
      });
  ...
  handlePagination = (page) => {
    ...
    // let urlPrefix = "archive/organizations";
    ...
    return (
      <React.Fragment>
        ...
        >
          ...
          <ModalBody>Do you really want to restore organization(s)?</ModalBody>
          <ModalFooter>
