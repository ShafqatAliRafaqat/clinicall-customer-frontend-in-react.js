...
import Sidebar from "./DoctorForm";
import SidebarView from "./DoctorView";
...
class RoleList extends Component {
    ...
    columns: [
      {
        ...
        cell: (row) => (
          <span
            ...
            onClick={() => {
              history.push(`/profile_settings?id=${row.id}`);
            }}
          >
            <Avatar
  ...
  getDoctors = async (page) => {
    try {
      let url = "auth/doctors?page=1";
      if (page) url = `auth/doctors?page=${page}`;
      this.setState({
        isLoading: true,
      });
  ...
  handlePagination = (page) => {
    let urlPrefix = "/doctors";
    history.push(`${urlPrefix}?page=${page.selected + 1}`);
    this.setState({ currentPage: page.selected });
    this.getDoctors(page.selected + 1);
  };
    ...
    return (
      <React.Fragment>
        ...
        {isLoading ? (
          ...
          >
            <DataTable
              ...
              pagination
              onRowClicked={(e) => {
                history.push(`/profile_settings?id=${e.id}`);
              }}
              paginationServer
              paginationComponent={() => (
                <ReactPaginate
        ...
        >
          ...
          <ModalBody>Do you really want to delete Doctor(s)?</ModalBody>
          <ModalFooter>
