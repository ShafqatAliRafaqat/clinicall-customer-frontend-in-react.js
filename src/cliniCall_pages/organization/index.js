...
import Sidebar from "./OrganizationForm";
import SidebarView from "./OrganizationView";
  ...
  return (
    <div>
      <Breadcrumbs
        ...
        breadCrumbActive="Organizations"
      />
      <FilterHeader
...
class OrganizationList extends Component {
    ...
    columns: [
      ...
      },
      {
        ...
        cell: (row) => (
          ...
          >
            {row.city_id.name},{row.country_code.name}
          </p>
        ),
      },
      {
  ...
  initiateFilterObject = () => {
    this.setState(
      ...
      () => {
        ...
        this.getOrganizations(currentPage, filterUrl);
      }
    );
  };
  ...
  getOrganizations = async (page) => {
    try {
      const { filterUrl } = this.state;
      let url = "admin/organization?page=1";
      if (page) url = `admin/organization?page=${page}`;
      if (filterUrl) url = `admin/organization?page=${page}&${filterUrl}`;
      console.log("this is api url with filter", url);
      this.setState({
        isLoading: true,
      });
  ...
  handlePagination = (page) => {
    let urlPrefix = "/organizations";
    history.push(`${urlPrefix}?page=${page.selected + 1}`);
    this.setState({ currentPage: page.selected });
    this.getOrganizations(page.selected + 1);
  };
