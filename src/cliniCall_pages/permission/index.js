...
import Sidebar from "./PermissionForm";
...
import SidebarView from "./PermissionView";
...
class PermissionList extends Component {
  ...
  getPermissions = async (page) => {
    try {
      let url = "admin/permissions?page=1";
      if (page) url = `admin/permissions?page=${page}`;
      this.setState({
        isLoading: true,
      });
  ...
  handleDelete = async () => {
    try {
      this.confirmDeleteModalToggle();
      let url = "admin/delete_permissions";
      this.setState({
        isLoading: true,
      });
  ...
  handlePagination = (page) => {
    ...
    let urlPrefix = "/permissions";
    history.push(`${urlPrefix}?page=${page.selected + 1}`);
    this.setState({ currentPage: page.selected });
    this.getPermissions(page.selected + 1);
  };
    ...
    return (
      <React.Fragment>
        ...
        >
          ...
          <ModalBody>Do you really want to delete Permission(s)?</ModalBody>
          <ModalFooter>
