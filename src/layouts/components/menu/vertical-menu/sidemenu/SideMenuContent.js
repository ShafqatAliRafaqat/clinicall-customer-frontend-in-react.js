...
class SideMenuContent extends React.Component {
  ...
  state = {
    ...
    currentActiveGroup: [],
    tempArr: [],
    navigations: this.props.userData.menu,
  };
  ...
  componentDidMount() {
    ...
    this.props.userData.menu.map((item) => {
      if (item.child_menu.length) {
        const children = [];
        item.child_menu.map((child) => {
          children.push({ ...child, type: "item", navLink: child.url });
        });
        nav.push({
          ...item,
          navLink: item.url,
  ...
  render() {
    ...
    const menuItems = this.state.navigations.map((item) => {
